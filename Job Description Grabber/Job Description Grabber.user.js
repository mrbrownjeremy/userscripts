...
(function () {
  ...
  function extractJobData() {
    ...
    // Reject values that look like search-input placeholder text or UI chrome
    // (e.g. LinkedIn's "City, state, or zip code … Clear search location").
    const lfLocation = lf(['location', 'job location', 'work location', 'office location']);
    if (lfLocation && !/zip\s*code|clear\s*search/i.test(lfLocation)) data.location = lfLocation;
    ...
    if (!data.salaryRange || !data.jobRefNum) {
      ...
      if (!data.salaryRange) {
        ...
        const salMatch =
          bodyText.match(new RegExp(`${cur}[\\d,]+(?:\\.\\d+)?k?(?:\\/(?:yr|year|hr|hour|wk|week|mo|month))?\\s+to\\s+${cur}[\\d,]+(?:\\.\\d+)?k?(?:\\/(?:yr|year|hr|hour|wk|week|mo|month))?${trailer}`, 'i')) ||
          bodyText.match(new RegExp(`between\\s+${cur}[\\d,]+(?:\\.\\d+)?k?\\s+and\\s+${cur}[\\d,]+(?:\\.\\d+)?k?`, 'i')) ||
          bodyText.match(new RegExp(`${cur}[\\d,]+(?:\\.\\d+)?k?(?:\\/(?:yr|year|hr|hour|wk|week|mo|month))?\\s*[-–—]\\s*${cur}?[\\d,]+(?:\\.\\d+)?k?(?:\\/(?:yr|year|hr|hour|wk|week|mo|month))?${trailer}`, 'i')) ||
          bodyText.match(new RegExp(`(?<!\\d)${bareNum}\\s+to\\s+${bareNum}(?!\\d)`, 'i')) ||
          bodyText.match(new RegExp(`(?<!\\d)${bareNum}\\s*[-–—]\\s*${bareNum}(?!\\d)`)) ||
          bodyText.match(new RegExp(`${cur}[\\d,]+(?:\\.\\d+)?k?(?:\\/(?:yr|year|hr|hour|wk|week|mo|month))?`, 'i'));
  ...
  function extractDescriptionSections(text) {
    ...
    // User phrases are prefix-matched: "What you'll do" catches "What you'll do at Figma" etc.
    const userRespExtra = getRespPhrases().map(p => escRe(p) + '.*').join('|');
    const userReqExtra  = getReqPhrases().map(p => escRe(p) + '.*').join('|');
  ...
  function buildPanel() {
    ...
    // Append to <html> not <body> — some sites (e.g. Greenhouse) mount React directly
    // on document.body, causing React reconciliation to evict non-virtual-DOM children.
  ...
  // CSS injected once (lazily) when the first modal opens — not at init time,
  // to avoid any risk of interfering with the host page during panel build.
  let _modalCssInjected = false;
  function injectModalCss() {
    if (_modalCssInjected) return;
  ...
  function doShowCodaModal() {
    ...
    dialog.innerHTML = `
      ...
      <div class="jdg-grid">
        ...
        <div class="jdg-field" id="jdg-industries-wrap">
          <div style="display:flex;align-items:baseline;justify-content:space-between;"><label>Industry(s)</label><a id="jdg-add-industry" href="#" style="font-size:10px;font-weight:400;color:#4a4adf;text-decoration:none;letter-spacing:0;">Add Industry</a></div>
        ...
        <div class="jdg-field"><div style="display:flex;align-items:baseline;justify-content:space-between;"><label>Channel</label><a id="jdg-add-channel" href="#" style="font-size:10px;font-weight:400;color:#4a4adf;text-decoration:none;letter-spacing:0;">Add Channel</a></div><select id="jdg-f-channel"><option value=""></option>${getChannelOptions().map(o => `<option value="${escHtml(o)}"${o === data.channel ? ' selected' : ''}>${escHtml(o)}</option>`).join('')}</select></div>
    ...
    dialog.querySelector('#jdg-add-industry').addEventListener('click', (e) => {
      e.preventDefault();
      doShowSettings();
    });
    dialog.querySelector('#jdg-add-channel').addEventListener('click', (e) => {
      e.preventDefault();
      doShowSiteModal();
    });
    ...
    dialog.innerHTML = `
      ...
      <div id="jdg-settings">
        ...
        <div class="jdg-settings-section">
          ...
          <p style="font-size:11px;color:#888;margin:0 0 8px;">Extra phrases to recognise as Responsibilities or Requirements headings. One phrase per line, plain text (no regex). Not case sensitive.</p>
          <label style="font-size:12px;font-weight:600;display:block;margin-bottom:4px;">Responsibilities</label>
          <textarea id="jdg-resp-phrases" rows="3" style="width:100%;box-sizing:border-box;font-size:12px;font-family:inherit;resize:vertical;margin-bottom:6px;border:1px solid #ccc;border-radius:4px;padding:4px 6px;" placeholder="e.g. What you'll be doing">${escHtml(currentRespPhrases)}</textarea>
  ...
  // Tampermonkey menu commands — work on any page regardless of allowlist
  GM_registerMenuCommand('🌐 Add current site to Job Description Grabber', () => {
    doShowSiteModal();
  });
  GM_registerMenuCommand('▶ Force show JDG panel', () => {
    const existing = document.getElementById('jdg-panel');
    if (existing) existing.remove();
    _lastExtractedData = null;
    _lastModalState = null;
  ...
  GM_registerMenuCommand('📤 Send to Coda', () => {
    const existing = document.getElementById('jdg-panel');
    if (!existing) {
      _lastExtractedData = null;
      _lastModalState = null;
  ...
  const navObserver = new MutationObserver(() => {
    ...
    if (_navTimer) return; // already scheduled — don't reset on every mutation
